...
export class FlightPhysics {
    ...
    step(dt, groundHeightFn) {
        ...
        const stalling = Math.abs(cl) > maxCL || Math.abs(aoa) > THREE.MathUtils.degToRad(16);
        if (stalling) {
            // Post-stall: lift drops off, drag rises sharply.
            const over = Math.min(1, (Math.abs(aoa) - THREE.MathUtils.degToRad(14)) / THREE.MathUtils.degToRad(10));
        ...
        if (!this.crashed) {
            ...
            const targetRollRate = this.controls.roll * sens.roll * maxDef.aileron * authoritySpeed * 2.2;
