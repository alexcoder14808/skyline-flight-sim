...
export class CameraManager {
    ...
    _updateLookFromKeys(dt) {
        ...
        // V2.1: clamp tighter than before (was +/-1.2 rad, ~69deg) — combined
        // with the aircraft's own pitch this could point the camera almost
        // straight up into flat, featureless sky, which reads as "broken"
        // even though it was technically rendering correctly.
        this.look.pitch = clamp(this.look.pitch + pitchInput * lookSpeed * dt, -0.85, 0.85);
    ...
    update(dt, aircraft) {
        ...
        if (this.mode === 'cockpit') {
            ...
            const lookQuat = root.quaternion.clone().multiply(
                new THREE.Quaternion().setFromEuler(new THREE.Euler(this.look.pitch * 0.7, this.look.yaw, 0, 'YXZ'))
            );
