...
## Current state (V2, as of this delivery)
...
effects or music — for either V1 or V2. Instead, every sound in the
simulator (`js/audio/AudioManager.js`) is generated at runtime with the Web
Audio API:
...
| music        | **V2**: a slow, generative 4-chord ambient pad (triangle-wave voices through independently-swept low-pass filters) with gentle auto-panning, plus a sparse randomized high bell/arpeggio layer for texture. Advanced each frame by `updateMusic(dt)`, called from `main.js`'s loop. |
